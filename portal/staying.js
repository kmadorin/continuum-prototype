/* Continuum Portal — staying.js
 * Investor — Staying (Rolling LP): privately rolls its position into the new
 * vehicle at the set price; approves the units leg. Sees its own election, the
 * public price, and the other LP redacted.
 */
"use strict";
(function () {
  const S = CT.state, ui = CT.ui, esc = S.esc, c = S.calc;
  const { fmtM, fmtUnits, pct } = S.fmt;
  const ROLE = "staying";          // rolling LP
  const OTHER = "leaving";
  const APPROVE_KEY = "staying";   // owns the units-delivery leg
        ...
        return ui.stageHead(ROLE, "Decide: roll or sell",
          ...
          + ui.twoCol(`<div class="card">
              ...
              <div class="choice-row" id="choice-row">
                <button class="choice" data-choice="roll" aria-pressed="true"><span class="ct">Roll over</span><span class="cd">Stay in — receive ${esc(d.vehicleShort)} units</span></button>
                <button class="choice" data-choice="exit" aria-pressed="false"><span class="ct">Sell</span><span class="cd">Take ${pct(c.buyPrice())} of NAV — receive USDC</span></button>
              </div>
              <div class="form-row"><label for="el-amt">Amount to roll — of ${fmtM(me.nav)} NAV (max)</label>
                <div class="input-group"><span class="prefix">$</span><input class="input num" id="el-amt" type="number" min="0" max="${me.nav}" step="0.1" value="${d.staying.roll.toFixed(1)}"><span class="suffix">M NAV</span></div>
              </div>
  ...
  function approveView(s) {
    ...
    return ui.stageHead(ROLE, "Approve my part",
      ...
      + (approved ? "" : ui.yourMove("Approve your units-delivery leg."))
      + actionCard + list;
  }
  ...
  function wire(root) {
    ...
    if (sub) sub.addEventListener("click", () => {
      const choice = root.querySelector('.choice[aria-pressed="true"]')?.dataset.choice || "roll";
      const amt = parseFloat(root.querySelector("#el-amt")?.value);
      S.actions.submitElection({ role: ROLE, choice, amount: amt });
      CT.shell.toast(`Election submitted privately — ${fmtM(Number.isFinite(amt) ? amt : S.deal().staying.roll)} NAV.`);
    });
  ...
  CT.page.run({ role: ROLE, workspace, wire, dashCards,
    dashIntro: () => ({ title: "Your fund interests", lede: "You decide privately whether to roll into the new vehicle or sell at the set price — and approve only your own leg. No other LP sees your choice." }) });
})();
